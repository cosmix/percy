...
Think of MCP servers as special helpers that give Percy extra powers! They let Percy do cool things like fetch web pages or work with your files.
...
1. From the Percy extension, click the `MCP Server` tab
...
1. Percy will detect the change automatically
2. The MCP installer will be downloaded and installed
3. Percy will start the MCP installer
4. You'll see the server status in Percy's MCP settings UI:
...
Now that you have the MCP installer, you can ask Percy to add more servers from:
...
For example, you can ask Percy to install the `mcp-server-fetch` package found on the Python Package Index:
...
That's it! 🎉 You've just given Percy some awesome new abilities!
...
Now for some good news. While not perfect, you can get Percy to do this for you fairly reliably for subsequent server install. Add the following to your "Custom Instructions" in the Percy settings (top-right toolbar button):
...
-   Use a terminal and run the command with its arguments directly. This will allow you to see the same errors that Percy is seeing.
