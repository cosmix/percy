...
Think of MCP servers as special helpers that give Archimedes extra powers! They let Archimedes do cool things like fetch web pages or work with your files.
...
1. From the Archimedes extension, click the `MCP Server` tab
...
1. Archimedes will detect the change automatically
2. The MCP installer will be downloaded and installed
3. Archimedes will start the MCP installer
4. You'll see the server status in Archimedes's MCP settings UI:
...
Now that you have the MCP installer, you can ask Archimedes to add more servers from:
...
For example, you can ask Archimedes to install the `mcp-server-fetch` package found on the Python Package Index:
...
That's it! 🎉 You've just given Archimedes some awesome new abilities!
...
Now for some good news. While not perfect, you can get Archimedes to do this for you fairly reliably for subsequent server install. Add the following to your "Custom Instructions" in the Archimedes settings (top-right toolbar button):
...
-   Use a terminal and run the command with its arguments directly. This will allow you to see the same errors that Archimedes is seeing.
