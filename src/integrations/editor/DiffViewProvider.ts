...
export class DiffViewProvider {
	...
	async update(fileChangeResult: FileChangeResult | string, isFinal: boolean) {
		...
		// If we have change regions and this is our first update with content
		if (changedRegions.length > 0 && this.streamedLines.length === 0) {
			// Immediately scroll to the first changed region
			const firstChangeLine = changedRegions[0].startLine
